...
import 'package:flutter_application_1/services/Approver_Api/News_Approver_Update_Api.dart';
...
class Newsapprovaledit extends StatefulWidget {
  final int newsId;
  final Function fetch;

  Newsapprovaledit({super.key, required this.newsId, required this.fetch});

  @override
  State<Newsapprovaledit> createState() => _NewsApprovalEdit();
}

class _NewsApprovalEdit extends State<Newsapprovaledit> {
...
// Ensure default values
      ...
      NewsUpdateApproverModel newsapproverupdate = NewsUpdateApproverModel(
        id: news.id ?? 0,
      ...
      print("approver edit News: ${newsapproverupdate.toJson()}");

      File? fileToUploadObj =
          selectedFile != null ? File(selectedFile!.path!) : null;
      // bool success = await updateNewsWithFormData(updated, fileToUploadObj);
      bool success =
          await ApproverEditWithFormData(newsapproverupdate, fileToUploadObj);
      if (success) {
        //
        String snackBarMessage = '';
        if (UserSession().userType == 'superadmin') {
          snackBarMessage = 'News Approved Successfully!';
